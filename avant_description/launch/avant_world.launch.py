"""
This launch file launches multiple simulation tools and given map file for the simulation
"""
...
def generate_launch_description():
    ...
    os.environ['GAZEBO_MODEL_PATH'] = os.path.join(pkg_share, "maps")
    spawn_x_val = "0.0"
    spawn_y_val = '0.0'
    spawn_z_val = '0.0'
    spawn_yaw_val = '0.0'
    for arg in sys.argv:
        ...
        if arg.startswith("world_path:="):
            path = arg.split("/")
            world_path = os.path.join(pkg_share, path[-2], path[-1])
    ...
    path_following_node = launch_ros.actions.Node(
        package='path_following_server',
